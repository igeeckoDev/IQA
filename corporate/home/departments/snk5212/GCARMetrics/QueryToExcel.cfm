...
<cfif url.refPage eq "/departments/snk5212/GCARMetrics/qManager.cfm"
    ...
    <cfoutput query="getItems">
        ...
        SELECT Count(DISTINCT docID) as CountItem
        FROM GCAR_Metrics
        WHERE
...
<cfelse>
    ...
    Select Count(DISTINCT docID) as CARCount
    FROM GCAR_Metrics
    WHERE
    <cfinclude template="shared/incProgramQuery.cfm">
        <cfif url.refPage eq "/departments/snk5212/GCARMetrics/qManager.cfm">
        ...
        AND #url.var6# = '#trim(url.var6Value)#'
            <cfif NOT isDefined("URL.FullTable")>
            AND #url.var7# = '#trim(url.var7Value)#'
            </cfif>
        </cfif>
    <cfinclude template="shared/incManagerAndViewQuery.cfm">
        <cfif isDefined("URL.Year")>
...
<td><b>Response Acceptance Time (Days)<b></td>
