...
<cfset subTitle = "CAR Trend Reports - Change CC / FYI Contacts - <a href='Report_Details.cfm?ID=#URL.ID#'>#Report.Function#</a>">
...
<cfif isDefined("Form.Submit") AND Form.Submit eq "Add Contact to Current List"
    ...
    <cfquery name="Report" datasource="UL06046"  username="#OracleDB_Username#" password="#OracleDB_Password#">
    SELECT Function, FunctionField, TopItems, SortField, TopItems_CARSource, CC
    FROM GCAR_METRICS_QREPORTS
    WHERE ID = #URL.ID#
    </cfquery>
    ...
    CC = '#ListAppend(Report.CC, newOwner)#'
...
<cfoutput>
	<cfif len(ownerList)>
        <b>New CC / FYI List</b> <span class="warning">Not Yet Saved!</span><br />
        #replace(ownerList, ",", "<br />", "All")#<br /><br />
    </cfif>
    
    <!---
	<cfloop from="1" to="#ListLen(ownerList)#" index="ListItem">
    	<cfoutput>#ListGetAt(ownerList, ListItem)# [<a href="Report_RemoveFYI.cfm?#cgi.QUERY_STRING#&ListItem=#ListItem#">remove</a>]<br /></cfoutput>
    </cfloop><br />
	--->
</cfoutput>

<cfquery name="Report" datasource="UL06046"  username="#OracleDB_Username#" password="#OracleDB_Password#">
SELECT Function, FunctionField, TopItems, SortField, TopItems_CARSource, CC, ID
FROM GCAR_METRICS_QREPORTS
WHERE ID = #URL.ID#
</cfquery>
...
<cfoutput query="Report">
    ...
    <b>Current CC / FYI</b>:
    <cfif ListLen(CC) gt 0>
    	[<a href="Report_RemoveFYI.cfm?#cgi.QUERY_STRING#&List=#CC#&ListItem=All">Remove All Contacts</a>]<br />
        <cfloop from="1" to="#ListLen(CC)#" index="CCListItem">
	        #ListGetAt(CC, CCListItem)# [<a href="Report_RemoveFYI.cfm?#cgi.QUERY_STRING#&List=#CC#&ListItem=#CCListItem#">remove</a>]<Br />
        </cfloop>
    <cfelse>
    	None<br />
    </cfif><br />    
    
	<!---<cfif len(CC)><br />#replace(CC, ",", "<br />", "All")#<cfelse>None Listed</cfif><br /><Br />--->
    ...
    <b>Select New CC / FYI Contact(s)</b><br />
    External UL Email Addresses Only<Br>
    :: <a href="javascript:popUp('EmailLookup.cfm?ID=#ID#&ownerList=#ownerList#&pageName=Report_ChangeFYI.cfm')">Lookup UL Email</a><br><br>
...
1. <b>Remove Names</b> - You can remove names from the <u>Current CC / FYI</u> list by selecting <u>remove</u> next to the appropriate name. You can also remove all names by selectiong <u>Remove All Contacts</u>.<br /><br />

2. <b>Add Names</b> - You can add a new name to the <u>Current CC / FYI</u> list by selecting <u>Lookup UL Email</u>, selecting a name, then selecting the <u>Add Contact to Current List</u> button. The new names are immediately saved to the <u>Current CC / FYI</u> list.<br /><br />

3. <b>Create New List</b> - You can create a <u>New CC / FYI</u> list by selecting <u>Lookup UL Email</u>, selecting a name, then selecting the <u>Add Contact to New List</u> button. You can continue to add names to the <u>New CC / FYI</u> list in this fashion. <span class="warning">Note - the new list is NOT saved until you complete Step 4</span><br /><br />

4. <b>Save New List</b> - In order to save the <u>New CC / FYI</u> list, you must select the <u>Save New List</u> button.<br /><br />
