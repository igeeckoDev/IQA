...
<cffunction name="genSalt" access="public" returnType="any" output="no">
   <cfargument name="size" type="numeric" required="false" default="12" hint="How many bytes should be used to generate the salt" />
...
<CFQUERY BLOCKFACTOR="100" NAME="pwd" DataSource="Corporate">
...
<cfoutput query="pwd">
	...
	<!--- generate salt for salt field --->
	<cfset salt = genSalt() />
	<!--- generate sha-512 hash for passwordnew field --->
	<cfset passwordHash = computeHash(password,salt) />
