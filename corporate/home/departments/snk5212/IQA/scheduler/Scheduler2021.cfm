<!--- This file is run the first day of every month at 6am by the cf scheduler / coldfusion server.
...
<cfset siteContacts = "">
<cfset AllAuditContactsTo = "">
<cfset AllAuditContactsBCC = "Global.InternalQuality@ul.com">
...
SELECT
	AuditSchedule.ID, AuditSchedule.AuditedBy, AuditSchedule.Email, AuditSchedule.Email2, AuditSchedule.AuditArea, AuditSchedule.Year_ as Year,
	AuditSchedule.OfficeName, AuditSchedule.AuditType, AuditSchedule.AuditType2, AuditSchedule.Month, AuditSchedule.Area, AuditSchedule.Status,
	AuditSchedule.Approved, AuditSchedule.LeadAuditor, AuditSchedule.Auditor, AuditSchedule.AuditorInTraining, AuditSchedule.SME,
...
<cfmail
            ...
            subject="AutoScope check">
                                
                This is to confirm Monthly scheduler works.
 </cfmail>
 </CFoutput>
...
<cfif len(SME)>
	<cfset AuditorCCEmails = listAppend(AuditorCCEmails, "#SME#")>
</cfif>
...
<cfif AuditType2 eq "Program">
	<!--- 
	program contacts removed may 8 2017 
	...
	<cfif AuditArea eq 'Scheme Documentation Audit'>
		<cfset incSubject = "#incSubject# (Scheme Documentation Audit)">
	</cfif>
...
<cfelseif AuditType2 is "Local Function"
    ...
    or audittype2 is "Local Function CBTL">
        ...
        <cfif isDefined("Area")>
			...
			<cfif AuditArea eq "Certification Body (CB) Audit">
				<cfset incSubject = "#incSubject# (Certification Body (CB) Audit)">
			</cfif>
...
<cfelseif AuditType2 is "Corporate">
	<!--- 
	corporate process owner - removed may 8 2017
...
<cfelseif AuditType2 is "Global Function/Process">
	<!--- 
	global process owner - removed may 8 2017
...
No action is required on your part at this time unless you foresee a scheduling conflict. The assigned lead auditor will contact you with specific scope information and to arrange the dates of the audit. The lead auditor assigned to this audit is <b>#LeadAuditor#</b> should you have any further questions.<br><br>
	...
	<cfif AuditType2 is "Local Function">
	    Quality Managers - Please contact #LeadAuditor# with your location's accreditation information.<br /><br />
    </cfif>
</cfmail>
