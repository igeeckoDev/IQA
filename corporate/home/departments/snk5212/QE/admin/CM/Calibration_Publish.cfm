...
SELECT Name, Email FROM IQADB_LOGIN "LOGIN" 
WHERE IQA = 'Yes'
AND Status IS NULL
ORDER BY Name
</cfquery>
...
<cfmail to="Internal.Quality_Audits@ul.com" from="Internal.Quality_Audits@ul.com" subject="#DB# Calibration Meeting Action Item #ItemID# has been assigned to you" type="HTML">
...
<b>Note</b>: You must be logged in to the CAR Process Website to view this link.<br><br>

<a href="http://#CGI.Server_Name#/departments/snk5212/QE/admin/CM/Calibration_Item.cfm?ID=#ID#">View</a> Action Item #ItemID#.
</div>
</cfmail>
</cfoutput>
...
<cfset i = 1>
<cfoutput query="Query">
#i# : Action Item #ItemID# sent to <cfif Owner eq "All">#EmailList#<cfelse>#Owner#</cfif><br>
<cfset i = i+1>
</cfoutput>
